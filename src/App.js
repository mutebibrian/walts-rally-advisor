...
export default function WaltsRallyAdvisor() {
  ...
  const [letterStep, setLetterStep] = useState("form"); // "form" | "draft" | "generating" | "document" | "sending" | "sent"
  ...
  const [letterPdfBase64, setLetterPdfBase64] = useState(null);
  const [letterPdfBlobUrl, setLetterPdfBlobUrl] = useState(null);
...
Keep it concise (3 short paragraphs max), factual, and reference the applicable NCR article number(s) for retirement/restart procedure.`;
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      setLetterPdfBase64(pdfData.pdfBase64);
      const blob = base64ToBlob(pdfData.pdfBase64, "application/pdf");
      setLetterPdfBlobUrl(URL.createObjectURL(blob));
      setLetterStep("document");
    } catch (err) {
      setLetterError(`Connection error: ${err.message}`);
      setLetterStep("draft");
    }
  ...
  const letterFileName = () => `Restart_Request_Car${letterFields.carNumber || ""}.pdf`;

  const getLetterFile = () => {
    const blob = base64ToBlob(letterPdfBase64, "application/pdf");
    return new File([blob], letterFileName(), { type: "application/pdf" });
  };

  const downloadPdf = () => {
    const a = document.createElement("a");
    a.href = letterPdfBlobUrl;
    a.download = letterFileName();
    a.click();
  };

  const shareLetter = async (platform) => {
    const shareText = `Restart request letter for Car #${letterFields.carNumber} — ${letterFields.eventName || "rally"}`;
    const file = getLetterFile();

    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      // Opens the device's native share sheet — the person picks Email,
      // WhatsApp, or anything else themselves; the PDF goes along attached.
      try {
        await navigator.share({ files: [file], title: "Restart Request Letter", text: shareText });
        return;
      } catch (err) {
    ...
    // download the file, then open the platform's compose window so the
    // person can attach the just-downloaded file manually.
    downloadPdf();
    if (platform === "email") {
      window.location.href = `mailto:${letterFields.cocEmail || ""}?subject=${encodeURIComponent(`Restart Request - Car #${letterFields.carNumber}`)}&body=${encodeURIComponent(shareText + " (PDF downloaded — please attach it to this email)")}`;
    } else if (platform === "whatsapp") {
      window.open(`https://wa.me/?text=${encodeURIComponent(shareText + " (PDF downloaded to your device — please attach it in WhatsApp)")}`, "_blank");
    }
  };

  const sendToCoc = async () => {
    setLetterError("");
  ...
  const closeLetterBuilder = () => {
    if (letterPdfBlobUrl) URL.revokeObjectURL(letterPdfBlobUrl);
    setLetterOpen(false);
    setLetterStep("form");
    ...
    setLetterDraft("");
    setLetterError("");
    setLetterPdfBase64(null);
    setLetterPdfBlobUrl(null);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
          <button
            onClick={() => { setLetterOpen(true); setLetterStep("form"); }}
            title="Generate Restart Letter"
            style={{
              width: 44, height: 44, borderRadius: "50%", flexShrink: 0,
              background: "#fff", border: "1.5px solid #eee", cursor: "pointer",
              display: "flex", alignItems: "center", justifyContent: "center",
      ...
      {letterOpen && (
        <div style={{
          ...
          <div style={{
            ...
            {letterStep === "draft" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#666" }}>Review & edit the letter:</label>
                <textarea
                  value={letterDraft}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  }}>Generate Signed Letter</button>
                </div>
              </div>
            )}

            {letterStep === "generating" && (
            ...
            {letterStep === "document" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div
                  onClick={() => window.open(letterPdfBlobUrl, "_blank")}
                  style={{
                    display: "flex", alignItems: "center", gap: 12, padding: "14px 16px",
                    background: "#faf5f4", border: "1.5px solid #eee", borderRadius: 14, cursor: "pointer",
                  }}
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#888" }}>Tap to view · Signed restart letter</div>
                  </div>
                  <div style={{ fontSize: 12, color: "#C0392B", fontWeight: 700 }}>VIEW</div>
                </div>

                <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                  <button onClick={() => shareLetter("email")} title="Share via Email" style={{
                    ...
                    <span style={{ fontSize: 11, color: "#666", fontWeight: 600 }}>Email</span>
                  </button>
                  <button onClick={() => shareLetter("whatsapp")} title="Share via WhatsApp" style={{
                    display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
                    background: "none", border: "none", cursor: "pointer", padding: 8,
                  }}>
                    <div style={{
                      width: 48, height: 48, borderRadius: "50%", background: "#e8f9ee",
                      display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22,
                    }}>💬</div>
                    <span style={{ fontSize: 11, color: "#666", fontWeight: 600 }}>WhatsApp</span>
                  </button>
                  <button onClick={downloadPdf} title="Download" style={{
                    display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
                    background: "none", border: "none", cursor: "pointer", padding: 8,
                  }}>
                    <div style={{
                    ...
                    <span style={{ fontSize: 11, color: "#666", fontWeight: 600 }}>Download</span>
                  </button>
                </div>
                ...
                <div>
                  <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>
                    Or have Walts Rally Advisor email it directly to the CoC:
                  </div>
                  <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>To: <strong>{letterFields.cocEmail}</strong></div>
                  <button onClick={sendToCoc} style={{
                    width: "100%", background: "linear-gradient(135deg, #C0392B, #922B21)", color: "#fff",
                    border: "none", borderRadius: 12, padding: "12px", fontSize: 14, fontWeight: 700, cursor: "pointer",
                  }}>Send to CoC via Email Now</button>
                </div>

                <button onClick={() => setLetterStep("draft")} style={{
                  background: "none", border: "none", color: "#888", fontSize: 12, cursor: "pointer", textAlign: "center",
                }}>← Back to edit letter</button>
              </div>
            )}
